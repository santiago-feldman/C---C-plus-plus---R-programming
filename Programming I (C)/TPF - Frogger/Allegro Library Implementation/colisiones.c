...
rana_t check_charcos(rana_t rana, charco_t charcos[], int pasos, int* nivel, int *score)
{
    ...
    if (rana.cant_vida <= 0)						//Si se murió la rana 
    {
...
/* FUNCIÓN HIGHSCORE_CHECK: se analiza el highscore, si el nuevo score es mayor a éste entonces lo actualiza y devuelve el highscore */ 
int highscore_check (int score)
{
    ...
    if(c == EOF)										//Si el primer carácter es un terminador
    {
        fclose(highscore_f);
    ...
    else												//Si el primer carácter no es un terminador
    {
        while (c!= EOF)									//Mientras que no encuentra el terminador, pasa los carácteres y los convierte a un número entero
        {
                ent *= 10;           
                ent += c - ASCII;	 					//Transformación a ascii 
                c = fgetc (highscore_f);	
        }
        fclose(highscore_f);
    }
    if (score > ent )									//Si el score es más grande que el highscore
    {
            highscore_f = fopen  ("highscore.txt","w");
